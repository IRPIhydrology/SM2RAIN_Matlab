function [R,RMSE,NS,KGE,PPsim,SM]=SM2RAIN_Tpot2_E(data,PAR,NN,FIG,namefig)
% load(name);
D=data(1:end,1); SM=data(:,2); Pobs=data(1:end-1,3); Eobs=data(1:end-1,4);
Z=PAR(1);
a=PAR(2);
b=PAR(3);
Tpot=PAR(4);
...
SM=SWI_POT_NaN([D,SM],Tbase,Tpot);
...
if FIG==1
    ...
    set(gca,'Fontsize',13)
    hold on
    SM=SWI_POT_NaN([data(:,1:2)],Tbase,Tpot);
...
% SWI computation with variable T, see Brocca et al., 2019 (ESSD)
function [SWIout,TT]=SWI_POT_NaN(data,T,POT)
dt=zeros(length(data),1);
Kn=ones(length(data),1);
D=data(:,1);
SWI=data(:,2);
...
for i=2:length(SWI)
    dt=(D(i)-D(i-1));
    Kn(i)=Kn(i-1)./(Kn(i-1)+exp(-dt/Tupd));
    SWI(i)=SWI(i-1)+Kn(i).*(SSM(i)-SWI(i-1));
    Tupd=T.*SWI(i).^-POT;
    Kn(i)=Kn(i-1)./(Kn(i-1)+exp(-dt/Tupd));
    SWI(i)=SWI(i-1)+Kn(i).*(SSM(i)-SWI(i-1));
...
function [SWIout,TT]=SWI_POT(data,T,POT)
dt=zeros(length(data),1);
Kn=ones(length(data),1);
SWI=data(:,2);
TT=SWI(:,1).*NaN;
...
for i=2:length(data)
    ...
    Kn(i)=Kn(i-1)./(Kn(i-1)+exp(-dt/Tupd));
    SWI(i)=SWI(i-1)+Kn(i).*(data(i,2)-SWI(i-1));
    Tupd=T.*SWI(i).^-POT;
    Kn(i)=Kn(i-1)./(Kn(i-1)+exp(-dt/Tupd));
    SWI(i)=SWI(i-1)+Kn(i).*(data(i,2)-SWI(i-1));
    Tupd=T.*SWI(i).^-POT;
